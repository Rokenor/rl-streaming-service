...
export const createStream = (formValues) => async (dispatch, getState) => {
  ...
  const response = await streams.post('http://localhost:3001/streams', { ...formValues, userId });
...
export const editStream = (id, formValues) => async (dispatch) => {
  const response = await streams.patch(`http://localhost:3001/streams/${id}`, formValues);
